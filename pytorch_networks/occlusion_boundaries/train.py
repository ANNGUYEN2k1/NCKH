...
from modeling import deeplab, pidinet
import dataloader
...
with open(CONFIG_FILE_PATH) as fd:
    config_yaml = oyaml.full_load(fd)  # Returns an ordered dict. Used for printing
...
########################Seeding###############################
utils.seeding(config.train.seed)
...
if config.train.datasetsVal is not None:
    ...
    for dataset in config.train.datasetsTrain:
        train_x, train_y, valid_x, valid_y = utils.load_data(dataset.images, dataset.labels, config.train.percentageDataForTraining)
        db_train = dataloader.OutlinesDataset_(train_x, train_y,
                                        transform=[augs_train, augs_test])
        db_train = torch.utils.data.Subset(db_train, range(len(db_train)))
        db_train_list.append(db_train)
        if config.train.percentageDataForTraining != 1.0:
            db_val = dataloader.OutlinesDataset_(valid_x, valid_y,
                                            transform=[augs_train, augs_test])
            db_val = torch.utils.data.Subset(db_val, range(len(db_val)))
...
utils.display_image_grid(db_train, samples =10, fig = os.path.join(MODEL_LOG_DIR ,'train_data.png'))
...
elif config.train.model == 'pidi':
    pdcs = ['cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv']
    pdcs_notconvert = []
    for i in range(16):
        op = pdcs[i]
        pdcs_notconvert.append(utils.createConvFunc(op, theta =1))
    model = pidinet.PiDiNet(inplane=config.train.numInputChannels, outplane= config.train.numClasses, pdcs = pdcs_notconvert, training= True)
else:
    raise ValueError(
...
if config.train.continueTraining:
    ...
    if 'model_state_dict' in CHECKPOINT:
        ...
        print(colored('Continuing training from checkpoint...Loaded data from checkpoint: ', 'green')+colored(config.train.pathPrevCheckpoint,'red'))
...
for epoch in range(START_EPOCH, END_EPOCH):
    ...
    for iter_num, batch in enumerate(tqdm(trainLoader)):
        ...
        if config.train.model == 'unet':
            loss = criterion(outputs, labels.long().squeeze(1))
        elif config.train.model == 'deeplab_xception' or config.train.model == 'deeplab_resnet' or config.train.model == 'drn' or config.train.model == 'pidi':
    ...
    utils.display_image_grid(db_val,model, 10, device, os.path.join(MODEL_LOG_DIR ,'train_{}.png'.format(epoch)))
