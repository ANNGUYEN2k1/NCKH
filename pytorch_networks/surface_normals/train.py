...
from modeling import deeplab, NNET
...
if config.train.model == 'unet':
    ...
    params = [{"params": model.get_1x_lr_params(), "lr": float(config.train.optimAdam.learningRate) / 10},
    {"params": model.get_10x_lr_params(), "lr": float(config.train.optimAdam.learningRate)}]
...
elif config.train.lossFunc == 'UG_NLL_ours' or config.train.lossFunc == 'NLL_ours':
    criterion = loss_functions.compute_loss(config.train.lossFunc)
else:
