...
from api import depth_completion_api_v3 as depth_completion_api
...
if __name__ == '__main__':
    ...
    depthcomplete = depth_completion_api.DepthToDepthCompletion(
                                                                depthWeightsFile=config.depth.pathWeightsFile,      
                                                                ...
                                                                baseline=config.depth2depth.baseline,
                                                                f=config.depth2depth.f,
                                                                ...
                                                                depthinferenceHeight=config.depth.inferenceHeight,
                                                                depthinferenceWidth=config.depth.inferenceWidth,
    ...
    for dataset in config.files:
        EXT_LEFT_IMG = ['-left.jpg']  #'-rgb.jpg' - includes normals-rgb.jpg
        EXT_RIGHT_IMG = ['-right.jpg']
        # EXT_DEPTH_IMG = ['-depth-rectified.exr', '-transparent-depth-img.exr']
        EXT_DEPTH_GT = ['-depth-rectified.exr', '-opaque-depth-img.exr']
        EXT_MASK = ['-mask.png']

        for ext in EXT_LEFT_IMG:
            left_rgb_file_list += (sorted(glob.glob(os.path.join(dataset.left_image, '*' + ext))))
        for ext in EXT_RIGHT_IMG:
            right_rgb_file_list += (sorted(glob.glob(os.path.join(dataset.right_image, '*' + ext))))
        # for ext in EXT_DEPTH_IMG:
        #     depth_file_list += (sorted(glob.glob(os.path.join(dataset.depth, '*' + ext))))
        assert len(left_rgb_file_list) == len(right_rgb_file_list), (
            'number of left rgb ({}) and right rgb images ({}) are not equal'.format(len(left_rgb_file_list), len(right_rgb_file_list)))
        # assert len(left_rgb_file_list) == len(depth_file_list), (
        #     'number of rgb ({}) and depth images ({}) are not equal'.format(len(left_rgb_file_list), len(depth_file_list)))

        if dataset.masks is not None and dataset.masks != '':
            for ext in EXT_MASK:
                segmentation_masks_list += (sorted(glob.glob(os.path.join(dataset.masks, '*' + ext))))
            assert len(left_rgb_file_list) == len(segmentation_masks_list), (
                'number of rgb ({}) and masks ({}) are not equal'.format(len(left_rgb_file_list),
        ...
        if dataset.gt_depth is not None and dataset.gt_depth != '':
            ...
            assert len(left_rgb_file_list) == len(gt_depth_file_list), (
                'number of rgb ({}) and gt depth ({}) are not equal'.format(len(left_rgb_file_list),
                                                                            len(gt_depth_file_list)))

    print('Total Num of rgb_files:', len(left_rgb_file_list))
    ...
    assert len(left_rgb_file_list) > 0, ('No files found in given directories')
    ...
    for i in range(len(left_rgb_file_list)):

        # Run Depth Completion
        left_img = imageio.imread(left_rgb_file_list[i])
        right_img = imageio.imread(right_rgb_file_list[i])
        # input_depth = api_utils.exr_loader(depth_file_list[i], ndim=1)

        try:
            start = time.time()
            input_depth, output_depth, filtered_output_depth = depthcomplete.depth_completion(
                left_img,
                # input_depth,
                right_img,
        ...
        print('\nImage {:09d} / {}:'.format(i, len(left_rgb_file_list) - 1))
    ...
    a1_mean = round(a1_mean / len(left_rgb_file_list), 2)
    a2_mean = round(a2_mean / len(left_rgb_file_list), 2)
    a3_mean = round(a3_mean / len(left_rgb_file_list), 2)
    rmse_mean = round(rmse_mean / len(left_rgb_file_list), 3)
    abs_rel_mean = round(abs_rel_mean / len(left_rgb_file_list), 3)
    mae_mean = round(mae_mean / len(left_rgb_file_list), 3)
    sq_rel_mean = round(sq_rel_mean / len(left_rgb_file_list), 3)
