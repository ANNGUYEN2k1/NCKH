...
from .modeling.OcclusionWeghts import pidinet
from .modeling.SurfaceNormal import bae
from .modeling.MaskSegmentation import MKDCNet
from .modeling.Depth import hitnet
from .modeling import deeplab, deeplab_masks

class InferenceDepth():
    def __init__(self, depthWeightsFile, depthModel='middlebury', imgHeight=288, imgWidth=512, B= 0.06, f= 2000):
        if depthModel == 'middlebury':
            print('Creating HitNet {} model for input_depth and loading checkpoint'.format(depthModel))
            model_type = hitnet.ModelType.middlebury
        elif depthModel == 'flyingthings':
            print('Creating HitNet {} model for input_depth and loading checkpoint'.format(depthModel))
            model_type = hitnet.ModelType.flyingthings
        elif depthModel == 'eth3d':
            print('Creating HitNet {} model for input_depth and loading checkpoint'.format(depthModel))
            model_type = hitnet.ModelType.eth3d
        else:
            raise NotImplementedError(
                'Model may only be "middlebury", "flyingthings" or eth3d". Given model is: {}'.format(depthModel))

        # Load Checkpoint and its data (weights file)
        if not os.path.isfile(depthWeightsFile):
            raise ValueError('Invalid path to the given weights file in config. The file "{}" does not exist'.format(
                depthWeightsFile))
        self.cameraConfig = hitnet.CameraConfig(B, f)
        self.model = hitnet.HitNet(depthWeightsFile, model_type, self.cameraConfig)
        self.dimension = (imgWidth, imgHeight)

    def runOnNumpyImage(self, left_img, right_img):
        left_img = cv2.resize(left_img, self.dimension)
        right_img = cv2.resize(right_img, self.dimension)
        disparity_map = self.model(left_img, right_img)
        B = self.cameraConfig.baseline
        f = self.cameraConfig.f
        depth_map = B*f/disparity_map
        return depth_map
...
class InferenceOutlines():
    ...
    def __init__(self, outlinesWeightsFile, outlinesModel='deeplab_resnet', imgHeight=288, imgWidth=512):
        ...
        elif outlinesModel == 'pidi':
            print('Creating PiDiNet model for outlines and loading checkpoint')
            pdcs_notconvert = []
            pdcs = ['cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv']
            for i in range(16):
                pdcs_notconvert.append(pidinet.createConvFunc(pdcs[i], theta =1))
            self.model = pidinet.PiDiNet(inplane=60, pdcs = pdcs_notconvert, dil=24, convert= False)
        elif outlinesModel == 'pidi_cv':
            print('Creating PiDiNet model for outlines and loading checkpoint')
            pdcs = ['cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv', 'cd', 'ad', 'rd', 'cv']
            self.model = pidinet.PiDiNet(inplane=60, pdcs = pdcs, dil=24, convert= True)
        else:
            raise NotImplementedError(
        ...
        if 'model_state_dict' in checkpoint:
            ...
            if outlinesModel == 'pidi_cv':
                self.model.load_state_dict(pidinet.convert_pidinet(checkpoint['model_state_dict'], pdcs))
            else:
    ...
    def runOnNumpyImage(self, img, scaling_factor_power = 3, scaling_factor_mul = 1000):
        ...
        with torch.no_grad():
            ...
            SCALING_FACTOR_PWR = scaling_factor_power
            SCALING_FACTOR_MUL = scaling_factor_mul
            output_softmax = nn.Softmax(dim=1)(outputs).squeeze(0).cpu().numpy()
            predictions = predictions.squeeze(0).cpu().numpy()
            predictions_tmp = np.zeros(predictions.shape)
            predictions_tmp[predictions == 2] = 1
            size = 5
            kernel = np.ones((size, size), np.uint8)
            predictions_tmp = cv2.dilate(predictions_tmp.astype(np.uint8), kernel, iterations =1)
            # TEST - REMOVING UNCERTAIN VALUES FROM OUTLINE WEIGHTS
            # output_softmax[output_softmax < 0.3] = 1e-4
            output_softmax[1, :, :][predictions_tmp == 1] = 1e-4
            ...
            # occlusion_boundary_weight_rgb = cv2.cvtColor(occlusion_boundary_weight_rgb, cv2.COLOR_BGR2GRAY)
...
class InferenceNormals():
    ...
    def __init__(self, normalsWeightsFile, normalsModel='deeplab_resnet', imgHeight=288, imgWidth=512):
        ...
        elif normalsModel == 'baeGN':
            print('Creating EEAU_SNE_GN model for normals and loading checkpoint')
            self.model = bae.bae(sampling_ratio = 0.4, importance_ratio = 0.7, architecture = 'GN')
        elif normalsModel == 'baeBN':
            print('Creating EEAU_SNE_BN model for normals and loading checkpoint')
            self.model = bae.bae(sampling_ratio = 0.4, importance_ratio = 0.7, architecture = 'BN')            
        else:
            raise NotImplementedError(
    ...
    def runOnNumpyImage(self, img):
        ...
        with torch.no_grad():
            ...
            if self.name == 'baeGN' or self.name == 'baeBN':
                norm_out_list, _, _ = outputs
                norm_out = norm_out_list[-1]
                outputs = norm_out[:, :3, :, :]  # (B, 3, H, W)

            # outputs = nn.functional.normalize(outputs, p=2, dim=1)
            surface_normals = outputs.squeeze(0).cpu().numpy()
    ...
    def __init__(self, masksWeightsFile, masksModel='drn', imgHeight=288, imgWidth=512):
        ...
        if masksModel == 'deeplab_resnet':
            ...
            self.model = deeplab_masks.DeepLab(num_classes=2, backbone='drn', sync_bn=True,
                                               freeze_bn=False)  # output stride is 8 for drn
        elif masksModel == 'mkdc':
            print('Creating MKDCNet model for masks and loading checkpoint')
            self.model = MKDCNet.DeepSegNet()
        else:
            raise NotImplementedError(
