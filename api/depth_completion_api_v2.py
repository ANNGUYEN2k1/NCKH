...
from . import inference_models_v2, utils
...
class DepthToDepthCompletion(object):
    ...
    def __init__(
        ...
        self.inferenceNormals = inference_models_v2.InferenceNormals(normalsWeightsFile=normalsWeightsFile,
                                                                  normalsModel=normalsModel,
                                                                  imgHeight=normalsInferenceHeight,
                                                                  imgWidth=normalsInferenceWidth)

        self.inferenceOutlines = inference_models_v2.InferenceOutlines(outlinesWeightsFile=outlinesWeightsFile,
        ...
        if masksWeightsFile:
            self.inferenceMasks = inference_models_v2.InferenceMasks(masksWeightsFile=masksWeightsFile,
    ...
    def _complete_depth(self, input_image, input_depth, inertia_weight=1000, smoothness_weight=0.001, tangent_weight=1):
        ...
        mask_out = cv2.dilate(self.mask_predicted, kernel, iterations=1)
        mask_out = np.where(mask_out > 127, 1, 0)

        size = 11
        kernel = np.ones((size, size), np.uint8)
        mask = cv2.erode(self.mask_predicted, kernel, iterations=1)
        size = 3
        kernel = np.ones((size, size), np.uint8)
        ...
        self.occlusion_weight[mask_out == 0] = np.max(self.occlusion_weight)
