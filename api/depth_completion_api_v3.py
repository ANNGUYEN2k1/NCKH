...
from . import inference_models_v3, utils
...
class DepthToDepthCompletion(object):
    ...
    FOLDER_MAP = {
        ...
        'left-image': {
            'postfix': '-left-rgb.png',
            'folder-name': 'input-image'
        },
        'right-image': {
            'postfix': '-right-rgb.png',
            'folder-name': 'input-image'
        },
        'input-point-cloud': {
            'postfix': '-input-pointcloud.ply',
    ...
    def __init__(
            ...
            depthinferenceHeight=288,
            depthinferenceWidth=512,
        ...
        self.inferenceDepth = inference_models_v3.InferenceDepth(depthWeightsFile= depthWeightsFile,
                                                                depthModel=depthModel,
                                                                imgHeight=depthinferenceHeight,
                                                                imgWidth=depthinferenceWidth,
                                                                B=baseline,
                                                                f=f)

        self.inferenceNormals = inference_models_v3.InferenceNormals(normalsWeightsFile=normalsWeightsFile,
                                                                  normalsModel=normalsModel,
                                                                  imgHeight=normalsInferenceHeight,
                                                                  imgWidth=normalsInferenceWidth)

        self.inferenceOutlines = inference_models_v3.InferenceOutlines(outlinesWeightsFile=outlinesWeightsFile,
        ...
        if masksWeightsFile:
            self.inferenceMasks = inference_models_v3.InferenceMasks(masksWeightsFile=masksWeightsFile,
    ...
    def store_depth_completion_outputs(self, root_dir, files_prefix, min_depth=0.0, max_depth=1.5):
        ...
        left_image_dir = os.path.join(root_dir, self.FOLDER_MAP['left-image']['folder-name'])
        right_image_dir = os.path.join(root_dir, self.FOLDER_MAP['right-image']['folder-name'])
        ...
        # Save Input Image
        input_image_filename = '{:09d}'.format(files_prefix) + self.FOLDER_MAP['left-image']['postfix']
        input_image_filename = os.path.join(left_image_dir, input_image_filename)
        imageio.imwrite(input_image_filename, self.input_image)
        input_image_filename = '{:09d}'.format(files_prefix) + self.FOLDER_MAP['right-image']['postfix']
        input_image_filename = os.path.join(right_image_dir, input_image_filename)
        imageio.imwrite(input_image_filename, self.right_image)
        ...
        grid_image1 = np.concatenate(
            (self.input_image, self.right_image, self.surface_normals_rgb, self.outlines_rgb, self.occlusion_weight_rgb), 1)
        grid_image2 = np.concatenate(
            (orig_input_depth_rgb, input_depth_rgb, output_depth_rgb, gt_depth_rgb, masked_img), 1)
    ...
    def _modify_depth_delete_masks(self, rgb_image, input_depth, dilate_mask=True):
        ...
        self.mask_predicted = self.inferenceMasks.runOnNumpyImage(rgb_image, dilate_mask= dilate_mask)
    ...
    def _complete_depth(self, input_image, input_depth, inertia_weight=1000, smoothness_weight=0.001, tangent_weight=1):
        ...
        utils.save_uint16_png(self._PATH_TMP_INPUT_DEPTH, scaled_input_depth.astype(np.uint16))
        ...
        self.occlusion_weight, self.occlusion_weight_rgb, self.outlines_rgb = self.inferenceOutlines.runOnNumpyImage(
            input_image, scaling_factor_mul = SCALING_FACTOR_MUL)
        ...
        self.input_image = cv2.resize(input_image, (self.outputImgWidth, self.outputImgHeight),
                                      interpolation=cv2.INTER_LINEAR)

        size = 5
        kernel = np.ones((size, size), np.uint8)
        mask1 = cv2.dilate(self.mask_predicted, kernel, iterations=1)
        mask1 = np.where(mask1 > 127, 1, 0)

        size = 5
        kernel = np.ones((size, size), np.uint8)
        mask = cv2.dilate(self.mask_predicted, kernel, iterations=1)
        size = 13
        kernel = np.ones((size, size), np.uint8)
        ...
        edge = np.where(self.occlusion_weight_rgb < 200, 1, 0)
        edge_clear = edge - mask
        edge_clear = np.where(edge_clear > 0, 1, 0)

        edge_add = edge_clear - self.outlines_rgb[:,:,1]/255
        edge_add = np.where(edge_add > 0, 1, 0)

        self.occlusion_weight[edge_add == 1] = np.mean(self.occlusion_weight[self.outlines_rgb[:,:,1] == 255])*1.2
        self.occlusion_weight[edge_clear == 0] = np.max(self.occlusion_weight) 
        self.occlusion_weight[mask1 == 0] = np.max(self.occlusion_weight)
        
        self.occlusion_weight_rgb = (self.occlusion_weight / SCALING_FACTOR_MUL * 255).astype(np.uint8)
        self.occlusion_weight_rgb = cv2.applyColorMap(self.occlusion_weight_rgb, cv2.COLORMAP_OCEAN)
        self.occlusion_weight_rgb = cv2.cvtColor(self.occlusion_weight_rgb, cv2.COLOR_BGR2RGB)
    ...
    def depth_completion(self,
        ...
        self.input_image = left_image
        input_depth = self.inferenceDepth.runOnNumpyImage(left_image, right_image)
        self.right_image = cv2.resize(right_image, (self.outputImgWidth, self.outputImgHeight),
                                      interpolation=cv2.INTER_LINEAR)
        # Clean input depth
        ...
        return self.orig_input_depth, self.output_depth, self.filtered_output_depth
    ...
    def depth_completion_synthetic_centroid(self,
        ...
        # modify input depth
        self.input_depth = self._modify_synthetic_depth_delete_objects(self.orig_input_depth,
                                                                  json_file_path,
                                                                  variant_mask_path,
                                                                  centroid_width=centroid_width)

        # run depth2depth
        self.output_depth, self.filtered_output_depth = self.depth_completion(self.input_image,
                                                                         self.input_depth,
                                                                         inertia_weight=inertia_weight,
                                                                         smoothness_weight=smoothness_weight,
                                                                         tangent_weight=tangent_weight,
